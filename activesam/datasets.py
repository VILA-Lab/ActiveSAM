...
class PascalVOC20Dataset(BaseSegDataset):
    """Pascal VOC 2012 with the 20 foreground classes (no background)."""
    METAINFO = dict(
    ...
    def __init__(self, ann_file, img_suffix='.jpg', seg_map_suffix='.png',
                 reduce_zero_label=True, **kwargs) -> None:
        super().__init__(
...
class COCOObjectDataset(BaseSegDataset):
    """COCO-Object: 1 background class + the 80 MS COCO object classes."""
    METAINFO = dict(
...
class PascalContext60Dataset(BaseSegDataset):
    """Pascal Context with 59 classes + background."""
    METAINFO = dict(
    ...
    def __init__(self, ann_file: str, img_suffix='.jpg', seg_map_suffix='.png',
                 **kwargs) -> None:
        super().__init__(
            img_suffix=img_suffix, seg_map_suffix=seg_map_suffix,
            ann_file=ann_file, reduce_zero_label=False, **kwargs)


@DATASETS.register_module()
class PascalContext59Dataset(BaseSegDataset):
    """Pascal Context with 59 classes (no background)."""
    METAINFO = dict(
    ...
    def __init__(self, ann_file: str, img_suffix='.jpg', seg_map_suffix='.png',
        ...
        super().__init__(
            img_suffix=img_suffix, seg_map_suffix=seg_map_suffix,
            ann_file=ann_file, reduce_zero_label=reduce_zero_label, **kwargs)
