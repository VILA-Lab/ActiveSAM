...
class SAM3VLBackbone(nn.Module):
    ...
    def _forward_image_no_act_ckpt(self, samples):
        ...
        neck_out = self.vision_backbone.forward(samples)
        # Unpack: neck now returns 5 values (raw trunk features as last element)
        if len(neck_out) == 5:
            sam3_features, sam3_pos, sam2_features, sam2_pos, raw_trunk_features = neck_out
        else:
            sam3_features, sam3_pos, sam2_features, sam2_pos = neck_out
            raw_trunk_features = None
        ...
        # Expose raw trunk features for Approach 28 (1024-dim at 72×72)
        if raw_trunk_features is not None:
            output["raw_trunk_features"] = raw_trunk_features
